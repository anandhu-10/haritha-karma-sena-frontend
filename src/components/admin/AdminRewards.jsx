...
const AdminRewards = () => {
    ...
    const fetchLeaderboard = async () => {
        try {
            const { data } = await axios.get(`${(process.env.REACT_APP_API_URL || "https://haritha-karma-sena-backend.onrender.com")}/api/leaderboard`);
            setLeaderboard(data);
        } catch (error) {
    ...
    const fetchRewards = async () => {
        try {
            const { data } = await axios.get(`${(process.env.REACT_APP_API_URL || "https://haritha-karma-sena-backend.onrender.com")}/api/rewards`);
            setRewards(data);
        } catch (error) {
    ...
    const handleAddReward = async (e) => {
        ...
        try {
            ...
            await axios.post(`${(process.env.REACT_APP_API_URL || "https://haritha-karma-sena-backend.onrender.com")}/api/rewards`, {
    ...
    const handleDeleteReward = async (id) => {
        try {
            const token = localStorage.getItem("token");
            await axios.delete(`${(process.env.REACT_APP_API_URL || "https://haritha-karma-sena-backend.onrender.com")}/api/rewards/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchRewards();
        } catch (error) {
